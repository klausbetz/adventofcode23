...
FROM (SELECT splitted_amouts.*, red * green * blue AS power
      FROM (SELECT game_id,
                   MAX(least_amount) FILTER (WHERE cube_color = 'red')   AS red,
                   MAX(least_amount) FILTER (WHERE cube_color = 'green') AS green,
                   MAX(least_amount) FILTER (WHERE cube_color = 'blue')  AS blue
            FROM (SELECT game_id, MAX(cube_amount) AS least_amount, cube_color
                  FROM tmp_parsed_input t
                  GROUP BY game_id, cube_color
                  ORDER BY game_id) least_amounts
            GROUP BY game_id) splitted_amouts) power;
